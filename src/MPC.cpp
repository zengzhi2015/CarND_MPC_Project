...
// Note: the chosen N * dt should not be too small. It must at least cover the main dynamic
// process of the agent. On the other hand, it should not be too large, or the prediction will
// be non-sense.
// Therefore, I choose 1s for N*dt
// In addition, the dt should not be too small, or the agent cannot respond to the commands
// so frequently. On the other hand, dt should not be too large, or there may be precision loss.
// I have tried (N,dt) pairs such as (20,0.05) (10,0.05) (20,0.1) (10,0.2)...
size_t N = 10;
double dt = 0.1;
...
// These are start index or pointers to each vector
size_t x_start = 0;
...
//    weight = dv * 100 + 500;
    ...
    for (uint t = 1; t < N; t++) {
      ...
      // Here is an approximation of the polynomial
      // AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * x0 * x0;
      // AD<double> f0 = coeffs[0] + coeffs[1] * x0;
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * x0 * x0 + coeffs[3] * x0 * x0 * x0;
      // This is not the exact solution, but an approximation
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Extract the variables in the state vector for easy use
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
  ...
  // Set bounds for initial constraints
